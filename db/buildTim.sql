...
--
-- Dumping routines for database 'TIM'
--
/*!50003 DROP PROCEDURE IF EXISTS `spAddUser` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spAddUser`(IN firstName varchar(45), IN lastName varchar(45), IN uname varchar(45), IN pword varchar(45), IN role int)
BEGIN	
	INSERT INTO Users (firstName, lastName, userName, roleID, password)
    VALUES (firstName, lastName, uname, AES_ENCRYPT(pword, role));
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`shaanr`@`localhost` PROCEDURE `spCheckUsernameExists`(IN uname varchar(45))
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spInsertTimeRecord`(IN d int, IN u int, IN a int)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spUpdatePassword`(IN uid int, IN pword varchar(45))
BEGIN
	DECLARE keyword varchar(45) DEFAULT '';
    SELECT keyword INTO @keyword FROM Configuration;
	UPDATE Users 
    SET password = AES_ENCRYPT(pword,UNHEX(SHA2(@keyword,512)))
    WHERE userID = uid;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2016-05-09 13:37:15
